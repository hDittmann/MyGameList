...
function validateEmail(value) {
  return /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(String(value).trim()); // apparently this checks for email formats?????? what lol????
}
...
export default function LoginModal({ open, onClose }) {
  ...
  useEffect(() => {
    ...
    // selects email input when the thingy opens, keeping it incase we want it later
    // const t = setTimeout(() => {
    ...
    <div
      ...
      onKeyDown={(e) => {
        ...
        if (e.key === "Enter") {
          e.preventDefault();
          handleSignIn();
          // dont add one for create account
        }
      }}
      tabIndex={-1}
    >
